...
Protocol phases:
    0x01-0x02  Handshake     Client identifies itself, server responds with MO key
    0x10-0x12  Send (MO)     Sender -> Server Massey-Omura 3-pass to deliver ciphertext
    0x20-0x22  Receive (MO)  Server -> Receiver Massey-Omura 3-pass to deliver ciphertext
    0x30-0x35  Control       Errors, presence notifications, delivery status
"""
...
# with a per-client Massey-Omura public key.
HELLO = 0x01
WELCOME = 0x02
...
def pack_welcome(server_mo_pubkey_x, server_mo_pubkey_parity):
    payload = pack_point(server_mo_pubkey_x, server_mo_pubkey_parity)
    return pack_frame(WELCOME, payload)


def pack_mo_send_init(dest_pubkey_x, dest_pubkey_parity, j, c1_x, c1_parity):
...
def pack_mo_recv_init(session_id, sender_pubkey_x, sender_pubkey_parity, j, c1_x, c1_parity):
...
def unpack_mo_send_init(payload):
    ...
    j = payload[off]
    off += 1
    c1_x, c1_parity, _ = unpack_point(payload, off)
    return dest_x, dest_parity, j, c1_x, c1_parity
